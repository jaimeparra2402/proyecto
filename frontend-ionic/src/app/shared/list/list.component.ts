...
export class ListComponent {
  ...
  selectItem(item: any)        { this.onSelect.emit(item); }
  editItem(item: any)          { this.edit.emit(item); }
  deleteItem(item: any)        { this.delete.emit(item); }
  importSinglePlayer(item: any){ this.onImportPlayer.emit(item); }
}